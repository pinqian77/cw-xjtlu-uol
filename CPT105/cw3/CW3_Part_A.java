...
public class CW3_Part_A {
    ...
    public static Picture tilt(Picture picture) {
        ...
        // 旋转中心
        double col_c = 0.5*(width-1);
        double row_c = 0.5*(height-1);
        double a = Math.toRadians(330);
        double b = Math.toRadians(300);

        for (int col = 0; col< width; col++){
            for (int row = 0; row< height; row++){
                int col_t = (int)((col-col_c)*Math.cos(a)-(row-row_c)*Math.sin(a)+col_c); 
                int row_t = (int)((col-col_c)*Math.sin(b)+(row-row_c)*Math.cos(b)+row_c);
                if(col_t<width && col_t>=0 && row_t<height && row_t>=0){
                    res_pic.setColor(col, row, picture.getColor(col_t, row_t));
                }
            }
        }
